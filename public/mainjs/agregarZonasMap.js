...
function createZonaBBOX(gidEnBD) {//crea ua caja para los puntos seleccionado 
    ...
    for (var i = 0; i < data_ovi_csv.length; i++) {
        ...
        if (gidEnBD.indexOf((data_ovi_csv[i].gid).toString()) == -1) {
            //var puntosCSV_zona_bbox = [];
            let xlat = [], ylong = []
            for (var j = 0; j < data_ovi_csv[i].longitud.length; j++) {
                xlat[j] = parseFloat(data_ovi_csv[i].latitud[j])
                ylong[j] = parseFloat(data_ovi_csv[i].longitud[j])
            ...
            if (xlat.length > 1) {
                ...
                geoJSON_CSV_collection.push({
                    ...
                    "properties": {
                        "gid": data_ovi_csv[i].gid,
                        "nom_col": data_ovi_csv[i].nom_col
                    },
