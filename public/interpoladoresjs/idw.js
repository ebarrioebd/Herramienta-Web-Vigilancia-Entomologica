...
function Zi(centro, puntos, exp, maxD) {
    ...
    let s1 = 0 // new Array();
    let s2 = 0 // new Array();
    for (let i = 0; i < puntos.cantidad_huevos.length; i++) {
        d = Math.sqrt(Math.pow(parseFloat(puntos.longitud[i]) - centro[0], 2) + Math.pow(parseFloat(puntos.latitud[i]) - centro[1], 2)) * 100000; 
        s1 += puntos.cantidad_huevos[i] / Math.pow(d, exp);
        s2 += 1 / Math.pow(d, exp);
    }
    return s1 / s2
}
